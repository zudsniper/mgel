...
(defn get-tokens []
  (def tokens (edn/read-string (slurp "token.edn")))
  (try
...
(comment
  ...
  (def code "fbbc4b2cb09e537beda2e01c3c1f0069118feea3814831b2f4697df586fd7f7c")
  ...
  (defn get-oauth-token-2 [code]
    (-> (hc/post "https://api.challonge.com/oauth/token"
                 {:form-params {:grant_type "authorization_code"
                                ...
                                :code code
                                :redirect_uri "https://oauth.pstmn.io/v1/callback"}
                  :oauth-token (:access_token (get-oauth-token))
                  :as :json})
        :body))
